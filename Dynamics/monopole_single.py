...
##### work out taking average and summing outside loop 
        ...
        #print(np.shape(time))
        ac = acf(time, nlags=400)#_series[rep,:])
        #print(np.shape(ac))
...
x = np.linspace(1,len(power),len(power))[:int(.5*ind)]
y = (np.abs(power)**2)[:int(.5*ind)]

logx = np.log10(x[60:-100])
logy = np.log10(y[60:-100])
yerr = 0.2 * y; 
logyerr = yerr / y
...
np.savetxt(f'powerdata_{B_val}_{jobid}.txt', np.abs(power)**2)
